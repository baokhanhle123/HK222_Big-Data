...
records_per_household = 6 * 24 * 30 * 1 # 1 month of data with 10 minute intervals 
households = 10
...
for i in range(households):
    ...
    for j in range(records_per_household):    
        ...
        if datetime.datetime.fromtimestamp(record['Timestamp']).day == 25 and datetime.datetime.fromtimestamp(record['Timestamp']).hour == 0 and datetime.datetime.fromtimestamp(record['Timestamp']).minute == 0:
            dataset1month.append(record)
